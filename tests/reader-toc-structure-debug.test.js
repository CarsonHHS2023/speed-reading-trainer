...
test('TOC debug payload exposes raw nodes, anchors, metadata, and frontend derivations', () => {
  const page = tocPage();
  const payload = Integration.tocDebugPayload(page);
  assert.equal(payload.diagnostic_version, 'reader_toc_structure_debug_v1');
  ...
  assert.ok(Object.hasOwn(payload.toc_items[1], 'final_frontend_indent_percent'));
});
